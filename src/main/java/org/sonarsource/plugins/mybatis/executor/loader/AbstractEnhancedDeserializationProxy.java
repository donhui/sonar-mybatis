...
public abstract class AbstractEnhancedDeserializationProxy {
  ...
  private final Map<String, org.sonarsource.plugins.mybatis.executor.loader.ResultLoaderMap.LoadPair> unloadedProperties;
  ...
  protected AbstractEnhancedDeserializationProxy(Class<?> type, Map<String, org.sonarsource.plugins.mybatis.executor.loader.ResultLoaderMap.LoadPair> unloadedProperties,
        ...
        synchronized (this.reloadingPropertyLock) {
          if (!FINALIZE_METHOD.equals(methodName) && PropertyNamer.isProperty(methodName) && !reloadingProperty) {
            ...
            if (unloadedProperties.containsKey(propertyKey)) {
              final org.sonarsource.plugins.mybatis.executor.loader.ResultLoaderMap.LoadPair loadPair = unloadedProperties.remove(propertyKey);
              if (loadPair != null) {
                try {

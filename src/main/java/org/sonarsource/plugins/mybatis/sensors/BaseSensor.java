package org.sonarsource.plugins.mybatis.sensors;
...
import org.sonarsource.plugins.mybatis.issues.SqlIssue;
...
public class BaseSensor {
    ...
    protected static synchronized void addIssues(SensorContext context, final SqlIssuesList issues,
            final InputFile file, Integer startLine) throws IOException {
            ...
            for (final SqlIssue issue : fileIssues.getValue()) {
                try {
                    ...
                    if (issue.isExternal) {
                        ...
                        if (issue.getLine() > 0) {
                            location.at(main.selectLine(startLine + issue.getLine()));
                        }
                    ...
                    if (issue.getLine() > 0) {
                        loc.at(main.selectLine(startLine + issue.getLine()));
                    }

...
import org.sonarsource.plugins.mybatis.rules.RuleResultType;
import org.sonarsource.plugins.mybatis.rules.SqlRules;
import org.sonarsource.plugins.mybatis.rules.TextCheckType;
...
public enum PSSQLRules {
		...
		List<SqlRules> rules = new ArrayList<>();
		{
			...
			customRules.getRule()
					.addAll(Arrays.asList(getWaitForRule(),
							/* getSelectAllRule(), */ /* getInsertRule(), */ getOrderByRule(), getSargRule(),
							getNullComparisonRule(), getWhereWithOrVsUnionRule(), getUnionVsUnionALLRule(),
	...
	protected Rule getNullComparisonRule() {
		...
		childNull.getNames().getTextItem().add(ExprContext.class.getSimpleName());
	...
	protected Rule getUnionVsUnionALLRule() {
		...
		rImpl.getNames().getTextItem().add(Combine_clauseContext.class.getSimpleName());
		rImpl.setRuleMatchType(RuleMatchType.CLASS_ONLY);
		{
			RuleImplementation child = new RuleImplementation();
	...
	protected Rule getExistsVsInRule() {
		...
		childSubqueryContext.getNames().getTextItem().add(Select_stmtContext.class.getSimpleName());
	...
	protected Rule getSargRule() {
		...
		ruleFunctionCall.getNames().getTextItem().add(Func_callContext.class.getSimpleName());
		...
		predicateContextContainsLike.getNames().getTextItem().add(ExprContext.class.getSimpleName());
